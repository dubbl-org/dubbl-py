...
class StockTakes:
    ...
    def retrieve(self, stock_take_id: str) -> ResponseValue:
        return self._client.get(f"/stock-takes/{stock_take_id}")

    def update(self, stock_take_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return self._client.patch(f"/stock-takes/{stock_take_id}", json=body)

    def delete(self, stock_take_id: str) -> ResponseValue:
        return self._client.delete(f"/stock-takes/{stock_take_id}")

    def apply(self, stock_take_id: str) -> ResponseValue:
        return self._client.post(f"/stock-takes/{stock_take_id}/apply")

    def get_line(self, stock_take_id: str, line_id: str) -> ResponseValue:
        raise NotImplementedError(
            "The current v1 API exposes stock take line updates, but not direct retrieval by line id."
        )

    def update_line(self, stock_take_id: str, line_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return self._client.patch(f"/stock-takes/{stock_take_id}/lines/{line_id}", json=body)

    def delete_line(self, stock_take_id: str, line_id: str) -> ResponseValue:
        raise NotImplementedError(
            "The current v1 API exposes stock take line updates, but not direct deletion by line id."
        )
...
class AsyncStockTakes:
    ...
    async def retrieve(self, stock_take_id: str) -> ResponseValue:
        return await self._client.get(f"/stock-takes/{stock_take_id}")

    async def update(self, stock_take_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return await self._client.patch(f"/stock-takes/{stock_take_id}", json=body)

    async def delete(self, stock_take_id: str) -> ResponseValue:
        return await self._client.delete(f"/stock-takes/{stock_take_id}")

    async def apply(self, stock_take_id: str) -> ResponseValue:
        return await self._client.post(f"/stock-takes/{stock_take_id}/apply")

    async def get_line(self, stock_take_id: str, line_id: str) -> ResponseValue:
        raise NotImplementedError(
            "The current v1 API exposes stock take line updates, but not direct retrieval by line id."
        )

    async def update_line(self, stock_take_id: str, line_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return await self._client.patch(f"/stock-takes/{stock_take_id}/lines/{line_id}", json=body)

    async def delete_line(self, stock_take_id: str, line_id: str) -> ResponseValue:
        raise NotImplementedError(
            "The current v1 API exposes stock take line updates, but not direct deletion by line id."
        )
