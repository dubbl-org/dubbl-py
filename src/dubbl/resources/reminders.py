...
class Reminders:
    ...
    def retrieve(self, reminder_id: str) -> Any:
        return self._client.get(f"/reminders/{reminder_id}")

    def update(self, reminder_id: str, **kwargs: Any) -> Any:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return self._client.patch(f"/reminders/{reminder_id}", json=body)

    def delete(self, reminder_id: str) -> Any:
        return self._client.delete(f"/reminders/{reminder_id}")

    def get_logs(self, **params: Any) -> Any:
...
class AsyncReminders:
    ...
    async def retrieve(self, reminder_id: str) -> Any:
        return await self._client.get(f"/reminders/{reminder_id}")

    async def update(self, reminder_id: str, **kwargs: Any) -> Any:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return await self._client.patch(f"/reminders/{reminder_id}", json=body)

    async def delete(self, reminder_id: str) -> Any:
