...
class Portal:
    ...
    def list_access(self, **params: QueryValue) -> ResponseValue:
        return self._client.get("/portal/access", params={_to_camel(k): v for k, v in params.items() if v is not None})

    def delete_access(self, access_id: str) -> ResponseValue:
        return self._client.delete(f"/portal/access/{access_id}")

    def revoke_access(self, access_id: str) -> ResponseValue:
    ...
    def get_statements(self, token: str, **params: QueryValue) -> ResponseValue:
        return self._client.get(
            f"/portal/{token}/statements", params={_to_camel(k): v for k, v in params.items() if v is not None}
        )

    def list_payments(self, token: str) -> ResponseValue:
        return self._client.get(f"/portal/{token}/payments")

    def list_quotes(self, token: str) -> ResponseValue:
        return self._client.get(f"/portal/{token}/quotes")

    def accept_quote(self, token: str, quote_id: str) -> ResponseValue:
        return self._client.post(f"/portal/{token}/quotes/{quote_id}/accept")

    def get_details(self, token: str) -> ResponseValue:
        return self._client.get(f"/api/portal/{token}")

    def list_invoices(self, token: str) -> ResponseValue:
        return self._client.get(f"/api/portal/{token}/invoices")

    def get_invoice_pdf(self, token: str, invoice_id: str) -> ResponseValue:
        return self._client.get(f"/api/portal/{token}/invoices/{invoice_id}/pdf", raw_response=True)

    def approve_invoice(self, token: str, invoice_id: str) -> ResponseValue:
...
class AsyncPortal:
    ...
    async def list_access(self, **params: QueryValue) -> ResponseValue:
        return await self._client.get(
            "/portal/access", params={_to_camel(k): v for k, v in params.items() if v is not None}
        )

    async def delete_access(self, access_id: str) -> ResponseValue:
        return await self._client.delete(f"/portal/access/{access_id}")

    async def revoke_access(self, access_id: str) -> ResponseValue:
    ...
    async def get_statements(self, token: str, **params: QueryValue) -> ResponseValue:
        return await self._client.get(
            f"/portal/{token}/statements", params={_to_camel(k): v for k, v in params.items() if v is not None}
        )

    async def list_payments(self, token: str) -> ResponseValue:
        return await self._client.get(f"/portal/{token}/payments")

    async def list_quotes(self, token: str) -> ResponseValue:
        return await self._client.get(f"/portal/{token}/quotes")

    async def accept_quote(self, token: str, quote_id: str) -> ResponseValue:
        return await self._client.post(f"/portal/{token}/quotes/{quote_id}/accept")

    async def get_details(self, token: str) -> ResponseValue:
        return await self._client.get(f"/api/portal/{token}")

    async def list_invoices(self, token: str) -> ResponseValue:
        return await self._client.get(f"/api/portal/{token}/invoices")

    async def get_invoice_pdf(self, token: str, invoice_id: str) -> ResponseValue:
        return await self._client.get(f"/api/portal/{token}/invoices/{invoice_id}/pdf", raw_response=True)

    async def approve_invoice(self, token: str, invoice_id: str) -> ResponseValue:
