...
class Loans:
    ...
    def retrieve(self, loan_id: str) -> ResponseValue:
        return self._client.get(f"/loans/{loan_id}")

    def update(self, loan_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return self._client.patch(f"/loans/{loan_id}", json=body)

    def delete(self, loan_id: str) -> ResponseValue:
        return self._client.delete(f"/loans/{loan_id}")

    def post_payment(self, loan_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
...
class AsyncLoans:
    ...
    async def retrieve(self, loan_id: str) -> ResponseValue:
        return await self._client.get(f"/loans/{loan_id}")

    async def update(self, loan_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return await self._client.patch(f"/loans/{loan_id}", json=body)

    async def delete(self, loan_id: str) -> ResponseValue:
        return await self._client.delete(f"/loans/{loan_id}")

    async def post_payment(self, loan_id: str, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
