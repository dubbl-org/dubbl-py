...
class BankRules:
    ...
    def update(self, rule_id: str, **kwargs: JSONValue) -> ResponseValue:
        return self._client.patch(
            f"/bank-rules/{rule_id}", json={_to_camel(k): v for k, v in kwargs.items() if v is not None}
        )

    def delete(self, rule_id: str) -> ResponseValue:
        return self._client.delete(f"/bank-rules/{rule_id}")

    def get_suggestions(self, **params: QueryValue) -> ResponseValue:
...
class AsyncBankRules:
    ...
    async def create(self, **kwargs: JSONValue) -> ResponseValue:
        return await self._client.post(
            "/bank-rules", json={_to_camel(k): v for k, v in kwargs.items() if v is not None}
        )
    ...
    async def get_suggestions(self, **params: QueryValue) -> ResponseValue:
        return await self._client.get(
            "/bank-rules/suggestions", params={k: v for k, v in params.items() if v is not None}
        )
