...
class Attachments:
    ...
    def retrieve(self, attachment_id: str) -> ResponseValue:
        return self._client.get(f"/attachments/{attachment_id}")

    def delete(self, attachment_id: str) -> ResponseValue:
        return self._client.delete(f"/attachments/{attachment_id}")

    def presign(self, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return self._client.post("/attachments/presign", json=body)

    def get_upload_url(self, **kwargs: JSONValue) -> ResponseValue:
...
class AsyncAttachments:
    ...
    async def retrieve(self, attachment_id: str) -> ResponseValue:
        return await self._client.get(f"/attachments/{attachment_id}")

    async def delete(self, attachment_id: str) -> ResponseValue:
        return await self._client.delete(f"/attachments/{attachment_id}")

    async def presign(self, **kwargs: JSONValue) -> ResponseValue:
        body = {_to_camel(k): v for k, v in kwargs.items() if v is not None}
        return await self._client.post("/attachments/presign", json=body)

    async def get_upload_url(self, **kwargs: JSONValue) -> ResponseValue:
